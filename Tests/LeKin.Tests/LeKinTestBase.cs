...
namespace LeKin.Tests
{
    public abstract class LeKinTestBase : AbpIntegratedTestBase<LeKinTestModule>
    {
        ...
        protected void UsingDbContext(Action<LeKinDbContext> action)
        {
            UsingDbContext(AbpSession.TenantId, action);
        }

        protected Task UsingDbContextAsync(Func<LeKinDbContext, Task> action)
        {
            return UsingDbContextAsync(AbpSession.TenantId, action);
        }

        protected T UsingDbContext<T>(Func<LeKinDbContext, T> func)
        {
            return UsingDbContext(AbpSession.TenantId, func);
        }

        protected Task<T> UsingDbContextAsync<T>(Func<LeKinDbContext, Task<T>> func)
        {
            return UsingDbContextAsync(AbpSession.TenantId, func);
        }

        protected void UsingDbContext(int? tenantId, Action<LeKinDbContext> action)
        {
            using (UsingTenantId(tenantId))
            {
                using (var context = LocalIocManager.Resolve<LeKinDbContext>())
                {
        ...
        protected async Task UsingDbContextAsync(int? tenantId, Func<LeKinDbContext, Task> action)
        {
            using (UsingTenantId(tenantId))
            {
                using (var context = LocalIocManager.Resolve<LeKinDbContext>())
        ...
        protected T UsingDbContext<T>(int? tenantId, Func<LeKinDbContext, T> func)
        {
            T result;
        ...
        protected async Task<T> UsingDbContextAsync<T>(int? tenantId, Func<LeKinDbContext, Task<T>> func)
        {
            T result;
